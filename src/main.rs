...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    ...
    // Primeiro, vamos verificar o boot_info
    println!("-> BootInfo details:");
    println!("   physical_memory_offset: {:#x}", boot_info.physical_memory_offset);
    println!("   memory_map regions: {}", boot_info.memory_map.len());

    println!("-> calling gale_sys::init()");
    gale_sys::init();
    println!("-> passoi o gale_sys::init()");

    use x86_64::VirtAddr;
    use memory::LinkedListFrameAllocator;

    let phys_mem_offset = VirtAddr::new(boot_info.physical_memory_offset);
    println!("-> VirtAddr created: {:#x}", phys_mem_offset.as_u64());

    println!("-> calling memory::init()");
    // Tenta inicializar a memória
    let mapper = unsafe { memory::init(phys_mem_offset) };
    println!("-> memory::init() completed successfully");

    println!("-> calling LinkedListFrameAllocator::init()");
    unsafe { LinkedListFrameAllocator::init(phys_mem_offset.as_u64(), &boot_info.memory_map) };
    println!("-> LinkedListFrameAllocator::init() completed");
